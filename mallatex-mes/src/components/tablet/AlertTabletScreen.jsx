...
export function AlertTabletScreen({ onSubmit }) {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, maxWidth: 800, margin: '0 auto' }}>
        {TIPOS.map((t) => (
          <button key={t.id} onClick={() => setSelected(t)} style={{
            ...
            padding: 22, borderRadius: 14, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10,
            background: selected?.id === t.id ? brand.red : brand.white,
            color: selected?.id === t.id ? 'white' : brand.ink,
            cursor: 'pointer', minHeight: 140, transition: 'all .15s',
          }}>
            <div style={{ fontSize: 52, lineHeight: 1 }}>{t.emo}</div>
            <div style={{ ...fontDisplay, fontWeight: 900, fontSize: 14, lineHeight: 1.1, textTransform: 'uppercase', whiteSpace: 'pre-line', textAlign: 'center' }}>{t.label}</div>
          </button>
        ))}
      </div>
      <div style={{ maxWidth: 600, margin: '24px auto 0' }}>
        <BigButton variant="red" emoji="📨" disabled={!selected} onClick={() => selected && onSubmit(selected.id, selected.desc)}>Enviar aviso</BigButton>
      </div>
    </div>
  )
}
