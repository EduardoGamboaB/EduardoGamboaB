...
export default function OrdersScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try { setQuotes(await api.quotes()); } catch {}
    setRefreshing(false);
  }, []);
  useEffect(() => { load(); }, [load]);
  ...
  async function convert(q) {
    Alert.alert('Levantar pedido', `¿Convertir ${q.folio} en pedido?`, [
      ...
      { text: 'Levantar', onPress: async () => { try { await api.createOrder({ quoteId: q.id }); load(); } catch (e) { Alert.alert('Error', e.message); } } },
    ]);
  }
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={st.empty}>{tab === 'pedidos' ? 'Aún no has levantado pedidos.' : 'Aún no tienes cotizaciones.'}</Text>}
        renderItem={({ item }) => {
            ...
            <View style={st.card}>
              ...
              <View style={st.rowTop}>
                ...
                {tab === 'cotizaciones' && item.status === 'abierta' && (
                  <TouchableOpacity style={st.convert} onPress={() => convert(item)}><Text style={st.convertTxt}>→ Pedido</Text></TouchableOpacity>
                )}
              </View>
            </View>
          );
