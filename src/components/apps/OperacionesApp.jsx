...
// MT-PC-001/002/003 transversal · Víctor G. (Coordinador)
import { useState } from 'react'
import {
  BarChart3, RefreshCw, ShieldCheck, FileWarning, Archive, FileText,
  Download, Check, AlertCircle, Mail, Camera, Eye,
} from 'lucide-react'
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
            ...
            <Panel title="Material con defecto / sobrante para procesar en SAE" code="MT-PC-003 act. 49">
              <Table cols={['Pedido', 'Material', 'Tipo', 'Cantidad', 'Acción']}
                rows={[
                  ['VFGC 872', 'Malla Raschel 35% B',   'Defecto',     '3.5 m',  <Btn key="b1" variant="primary" icon={Mail}>Notificar proveedor</Btn>],
                  ['VFGC 861', 'Cubrenegro 105gr',      'Sobrante',    '12.5 m', <Btn key="b2" variant="ok" icon={Archive}>Retornar almacén</Btn>],
                  ['VEG 1082', 'Monofila Blanco',       'Desperdicio', '1.5 m',  <Btn key="b3" variant="ghost">Registrar</Btn>],
                ]} />
            </Panel>
          </div>
        </>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
            <div style={{ background: brand.white, border: `1px solid ${brand.line}`, padding: 18, borderRadius: 4, marginBottom: 20, borderLeft: `3px solid ${brand.red}` }}>
              ...
              <div style={{ ...fontBody, fontSize: 13, color: '#444' }}>
                Operaciones realiza los traspasos entre almacenes del sistema SAE: <b>Materia prima → Producción → Producto terminado</b>.
              </div>
            </div>
        ...
        <>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
            <div style={{ background: '#fff3cd', border: `1px solid ${brand.warn}`, padding: 14, borderRadius: 4, marginBottom: 20, ...fontBody, fontSize: 13, color: '#856404' }}>
              <b>Política:</b> Auditoría interna 2 veces por mes. Seleccionar caso aleatorio y verificar cumplimiento. Diferencias → aclaración con evidencia.
            </div>
            <Panel title="Auditorías recientes">
              <Table cols={['Fecha', 'Tipo', 'Caso auditado', 'Hallazgos', 'Cumplimiento', 'Acción']}
                rows={[
                  ...
                  ['2026-05-01', 'Producción', 'VFGC 832',      'Documentos completos',        <Badge key="3" variant="ok">95%</Badge>, 'Ninguna'],
                ]} />
            </Panel>
          </div>
        </>
