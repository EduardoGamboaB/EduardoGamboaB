...
import { LINEAS_REALES, ROLES, PROCESOS_PRODUCCION, CATEGORIAS_MATERIAL, ROLE_PROFILES, EMOJI_LINEA } from '@/mes/lib/constants'
...
import { Chip } from '@/mes/components/shell/Chip'
...
export function ProduccionApp() {
  ...
  return (
    <WebShell perfil="Producción" profileData={ROLE_PROFILES.produccion} currentView={view} onChangeView={setView} onExit={onExit} menuItems={menuItems}>
      {view === 'tablero' && (
        <>
              ...
              <Table cols={['Pedido', 'Fecha pedido', 'Cliente', 'Material', 'Rollos', 'Medida', 'M²', 'Status', 'Compromiso', 'Entrega', 'Terminados', 'Entregados', 'Pendientes', 'En piso', 'Línea/Op']}
                rows={orders.map((o) => {
                  ...
                  return [
                    ...
                    o.linea ? <span key="l" style={{ ...fontMono, fontSize: 11 }}>{EMOJI_LINEA[o.linea] || ''} {o.linea} · {ops.map((o) => o.name).join(',')}</span> : <span style={{ color: '#999' }}>—</span>,
                  ]
                })} />
            </Panel>
          </div>
        ...
        <>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
            <div style={{ background: '#e3f2fd', border: `1px solid #1565c0`, padding: 14, borderRadius: 8, marginBottom: 20, ...fontBody, fontSize: 13, color: '#0d47a1' }}>
              <b>⏱️ Reemplaza el archivo de productividad en Excel.</b> Registra hora real de inicio/final, imprevistos (min), tiempo efectivo, metros lineales y piezas por turno. Calcula automáticamente mL/hr y piezas/hr.
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 24 }}>
              ...
              <Stat label="Promedio planta" value={`${Math.round(operators.filter((o) => o.promedioMLhr).reduce((a, o) => a + (o.promedioMLhr ?? 0), 0) / operators.filter((o) => o.promedioMLhr).length)} mL/hr`} variant="ok" emoji="📈" />
            ...
            <Panel title="Promedio mL/hr por operador" emoji="🏆">
              <div style={{ padding: 16 }}>
                {operators.filter((o) => o.role === ROLES.OPERADOR && o.promedioMLhr).sort((a, b) => (b.promedioMLhr ?? 0) - (a.promedioMLhr ?? 0)).map((op) => {
                  const maxML = Math.max(...operators.filter((o) => o.promedioMLhr).map((o) => o.promedioMLhr ?? 0))
                  const pct = ((op.promedioMLhr ?? 0) / maxML) * 100
                  return (
                    <div key={op.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: `1px solid ${brand.line}` }}>
                      <div style={{ width: 32, height: 32, borderRadius: '50%', background: op.color, color: 'white', display: 'grid', placeItems: 'center', ...fontDisplay, fontWeight: 900, fontSize: 14 }}>{op.initial}</div>
                      <div style={{ minWidth: 120 }}>
                        <b style={{ ...fontDisplay, fontSize: 14, textTransform: 'uppercase' }}>{op.name}</b>
                        <div style={{ ...fontMono, fontSize: 10, color: '#666', textTransform: 'uppercase', marginTop: 2 }}>{EMOJI_LINEA[op.linea ?? '']} {op.linea} · Tipo {op.tipo}</div>
                      </div>
                      <div style={{ flex: 1, height: 14, background: '#eee', borderRadius: 7, overflow: 'hidden' }}>
                        <div style={{ width: `${pct}%`, height: '100%', background: `linear-gradient(90deg, ${brand.red}, ${brand.redDark})`, borderRadius: 7 }} />
                      </div>
                      <b style={{ ...fontMono, fontSize: 14, fontWeight: 700, minWidth: 80, textAlign: 'right' }}>{op.promedioMLhr} mL/hr</b>
                    </div>
                  )
                })}
              </div>
            </Panel>
            <Panel title="Registro diario por turno (últimos)" emoji="📋" code="MT-DT-003"
              filters={<><Chip on>Todos</Chip><Chip>Hoy</Chip><Chip>Esta semana</Chip></>}>
              <Table cols={['Fecha', 'Línea', 'Operador', 'Inicio', 'Final', 'Imprev.', 'T. efectivo', 'mL turno', 'Piezas', 'mL/hr', 'Pz/hr', 'Descripción']}
                rows={productividad.map((p) => {
                  ...
                  return [
                    ...
                    <span key="d" style={{ ...fontBody, fontSize: 11, color: '#666' }}>{p.descripcion?.slice(0, 60)}</span>,
                  ]
                })} />
            </Panel>
          </div>
