...
import { brand, fontDisplay, fontDisplayItalic, fontBody, fontMono } from '@/mes/lib/brand'
...
const PROFILES = [
  { id: 'tablet',      name: 'Tablet de Línea', desc: 'Tablet fija en la línea — operadores tocan piezas, escanean rollos y reportan', emoji: '📺', code: 'MT-PC-003' },
  { id: 'produccion',  name: 'Producción',      desc: 'Jefe de producción',                                                              emoji: '🏭', code: 'MT-PC-003' },
  { id: 'almacen',     name: 'Almacén',         desc: 'Egreso e ingreso',                                                                emoji: '📦', code: 'MT-PC-001/002' },
  { id: 'operaciones', name: 'Operaciones',     desc: 'Coordinador general',                                                             emoji: '⚙️', code: 'MT-PC-001/002/003' },
  { id: 'cobranza',    name: 'Cobranza',        desc: 'Libera pedidos a producción',                                                     emoji: '💰', code: '' },
  { id: 'director',    name: 'Dirección',       desc: 'KPIs ejecutivos · OEE',                                                           emoji: '📈', code: '' },
]
  ...
  return (
    <div style={{ minHeight: '100vh', background: brand.paper }}>
      <nav style={{ padding: '18px 32px', background: brand.white, borderBottom: `1px solid ${brand.line}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ ...fontMono, fontSize: 10, letterSpacing: '0.25em', color: '#888', textTransform: 'uppercase' }}>
          Planta Zapopan · Jal.
        </div>
      </nav>

      <div style={{ position: 'relative', height: 8, background: brand.red, overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: `linear-gradient(90deg, ${brand.red} 0%, ${brand.redDark} 50%, ${brand.red} 100%)` }} />
      </div>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '80px 32px', position: 'relative' }}>
        ...
        <p style={{ ...fontBody, fontSize: 17, color: '#555', maxWidth: 680, lineHeight: 1.5, marginBottom: 32 }}>
          Implementa los procesos formales <b>MT-PC-001</b> (Ingreso), <b>MT-PC-002</b> (Egreso) y <b>MT-PC-003</b> (Producción). Alineado al Manual de Organización <b>MT-MA-001-Admon</b>. 7 perfiles, datos simulados, indicadores integrados.
        </p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 16 }}>
          {PROFILES.map((p, i) => (
            ...
            >
              ...
              <div style={{ position: 'absolute', top: 18, right: 20, ...fontMono, fontSize: 10, color: '#bbb', letterSpacing: '0.15em', fontWeight: 600 }}>
                0{i + 1}
              </div>
              ...
              {p.code && (
                <div style={{ ...fontMono, fontSize: 9, letterSpacing: '0.15em', color: brand.red, fontWeight: 600, textTransform: 'uppercase' }}>
                  {p.code}
                </div>
              )}
            </button>
      ...
      <section style={{ background: brand.white, borderTop: `1px solid ${brand.line}`, padding: '60px 32px' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40, marginBottom: 48 }}>
            <div>
              <div style={{ ...fontMono, fontSize: 11, letterSpacing: '0.25em', color: brand.red, marginBottom: 10, textTransform: 'uppercase', fontWeight: 700 }}>
                ◆ Misión
              </div>
          ...
          <div style={{ marginBottom: 40 }}>
            <div style={{ ...fontMono, fontSize: 11, letterSpacing: '0.25em', color: brand.red, marginBottom: 14, textTransform: 'uppercase', fontWeight: 700 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 14 }}>
              {VALORES.map((val, i) => (
                <div key={i} style={{ background: brand.paper, padding: 18, borderRadius: 8, borderTop: `3px solid ${brand.red}`, position: 'relative' }}>
                  ...
                  <div style={{ ...fontDisplay, fontWeight: 800, fontSize: 14, textTransform: 'uppercase', marginBottom: 8, letterSpacing: '0.02em' }}>
                    {val.v}
                  </div>
