...
export function AlmacenApp() {
  ...
  return (
    <WebShell perfil="Almacén" profileData={ROLE_PROFILES.almacen} currentView={view} onChangeView={setView} onExit={onExit} menuItems={menuItems}>
      {view === 'panel' && (
        <>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
            <div style={{ background: brand.white, border: `1px solid ${brand.line}`, padding: 18, borderRadius: 4, marginBottom: 20, borderLeft: `3px solid ${brand.red}` }}>
              ...
              <div style={{ ...fontBody, fontSize: 13, color: '#444' }}>
                <b>1.</b> Verificar Packing List vs documentación · <b>2.</b> Etiquetar rollos con QR (medida, material, peso, lote, ubicación) · <b>3.</b> Muestreo 1 de cada 5 rollos · <b>4.</b> Pruebas visual, táctil y peso · <b>5.</b> Si pasa: registrar en SAE (max 1 día hábil)
              </div>
            </div>
        ...
        <>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
            <div style={{ background: '#fff3cd', border: `1px solid ${brand.warn}`, padding: 14, borderRadius: 4, marginBottom: 20, ...fontBody, fontSize: 13, color: '#856404' }}>
              <b>Procedimiento:</b> Seleccionar 1 de cada 5 rollos aleatoriamente. Si solo hay unidades únicas, pesar todas. Pruebas: visual (integridad), táctil (textura), peso (vs declarado). Diferencia detectada → segundo muestreo. Si persiste → limitar acceso.
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 24 }}>
              <div style={{ background: brand.white, border: `1px solid ${brand.line}`, padding: 18, borderRadius: 4, borderTop: `3px solid ${brand.red}`, textAlign: 'center' }}>
                <Eye size={28} style={{ margin: '0 auto 8px', color: brand.red }} />
                <div style={{ ...fontDisplay, fontWeight: 800, fontSize: 14, textTransform: 'uppercase' }}>Prueba visual</div>
                <div style={{ ...fontBody, fontSize: 12, color: '#666', marginTop: 4 }}>Integridad superficial</div>
              </div>
              <div style={{ background: brand.white, border: `1px solid ${brand.line}`, padding: 18, borderRadius: 4, borderTop: `3px solid ${brand.red}`, textAlign: 'center' }}>
                <Camera size={28} style={{ margin: '0 auto 8px', color: brand.red }} />
                <div style={{ ...fontDisplay, fontWeight: 800, fontSize: 14, textTransform: 'uppercase' }}>Prueba táctil</div>
                <div style={{ ...fontBody, fontSize: 12, color: '#666', marginTop: 4 }}>Textura y acabado</div>
              </div>
              <div style={{ background: brand.white, border: `1px solid ${brand.line}`, padding: 18, borderRadius: 4, borderTop: `3px solid ${brand.red}`, textAlign: 'center' }}>
                <Weight size={28} style={{ margin: '0 auto 8px', color: brand.red }} />
                <div style={{ ...fontDisplay, fontWeight: 800, fontSize: 14, textTransform: 'uppercase' }}>Prueba de peso</div>
                <div style={{ ...fontBody, fontSize: 12, color: '#666', marginTop: 4 }}>vs Packing List</div>
              </div>
            </div>
          </div>
        </>
        ...
        <>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
            <div style={{ background: brand.white, border: `1px solid ${brand.line}`, padding: 18, borderRadius: 4, marginBottom: 20, borderLeft: `3px solid ${brand.red}` }}>
              ...
              <div style={{ ...fontBody, fontSize: 13, color: '#444' }}>
                <b>Prioridad:</b> primero rollos empezados (excedentes de otros pedidos), después rollos nuevos. Verificar SAE vs físico antes de surtir. No surtir si el pedido no está liberado por Cobranza.
              </div>
            </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
            ...
            <Panel title="Todos los rollos">
              <Table cols={['QR', 'Material', 'Medida', 'Lote', 'Peso', 'Ubicación', 'Empezado', 'Pedido', 'Estado']}
                rows={rolls.map((r) => [
                  <span key="q" style={{ ...fontMono, fontSize: 10, color: '#666' }}>
                    <QrCode size={12} style={{ display: 'inline', marginRight: 4 }} />{r.id}
                  </span>,
