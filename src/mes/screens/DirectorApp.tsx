...
import { ROLES, ROLE_PROFILES, EMOJI_LINEA } from '@/mes/lib/constants'
...
export function DirectorApp() {
  ...
  return (
    <WebShell perfil="Dirección" profileData={ROLE_PROFILES.director} currentView={view} onChangeView={setView} onExit={onExit} menuItems={menuItems}>
      {view === 'ejecutivo' && (
        <>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
            ...
            <Panel title="Productividad por operador (datos reales del Excel)" code="MT-PC-003 § 6">
              <Table cols={['Operador', 'Tipo', 'Línea', 'Prom. mL/hr', 'Turnos', 'Cumplimiento']}
                rows={operators
                  .filter((o) => o.role === ROLES.OPERADOR && o.promedioMLhr)
                  .sort((a, b) => (b.promedioMLhr ?? 0) - (a.promedioMLhr ?? 0))
                  .map((op) => {
