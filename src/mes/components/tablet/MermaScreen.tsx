...
import { Counter } from '@/mes/components/ui/Counter'
import { Order } from '@/mes/lib/seed'
...
export function MermaScreen({
  ...
  order?: Order
  onSubmit: (metros: number, categoria: string, defecto: string | null) => void
}) {
    ...
    <div style={{ flex: 1, overflowY: 'auto', padding: 24, background: brand.paper, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
      <div>
        <div style={{ marginBottom: 14, background: brand.red, color: 'white', padding: 18, borderRadius: 12 }}>
          <div style={{ ...fontMono, fontSize: 10, letterSpacing: '0.25em', color: 'rgba(255,255,255,0.85)', textTransform: 'uppercase', marginBottom: 6, fontWeight: 700 }}>
            Orden actual
          </div>
      ...
      <div>
        ...
        <BigButton variant="ghost" emoji="📷" onClick={() => alert('Aquí se abriría la cámara')}>
          Tomar foto (evidencia)
        </BigButton>
        <BigButton
          variant="red"
          emoji="💾"
          disabled={!metros || !categoria || (categoria !== 'sobrante' && !defecto)}
          onClick={() => categoria && onSubmit(metros, categoria, defecto)}
        >
          Guardar registro
        </BigButton>
      </div>
